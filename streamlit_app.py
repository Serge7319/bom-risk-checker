...
# Global Cadivor application shell.
# Single source of truth for the fixed topbar/sidebar and page offset.
# Important: this replaces the accumulated Milestone 4 shell patches. The large
# top gap was caused by many separate Streamlit markdown/style rows being
# rendered before the dashboard. We collapse style-only rows and render the app
# chrome once.

# Load reusable component styling first; the shell CSS below wins any layout conflicts.
inject_premium_css()
inject_v32_ux_css()
...
for _nav in NAV_OPTIONS:
    ...
    _nav_html.append(
        f'<a class="cv-side-link{_active}" href="{_href}" target="_self"><span>{_nav_icons.get(_nav,"•")}</span>{_nav}</a>'
    )

_topbar_avatar = shell_initials

st.markdown(
    f"""
    <style id="cadivor-shell-v10-root-fix">
    ...
    /* Critical fix: Streamlit creates a row for every st.markdown(<style>...</style>).
       Earlier milestones injected many style rows before the real page. These rows
       were visually empty but still reserved vertical space. Collapse style-only rows. */
    .element-container:has(div[data-testid="stMarkdownContainer"] > style:only-child),
    .element-container:has(div[data-testid="stMarkdownContainer"] style#cadivor-shell-v10-root-fix),
    ...
    /* The shell HTML is fixed-position, so its Streamlit wrapper must not reserve height. */
    ...
    /* Main content owns the only intentional offset. */
    ...
    .cadivor-topbar {{
        ...
        z-index:1000005!important; display:grid!important;
        grid-template-columns:var(--cv-sidebar-width) minmax(360px, 1fr) auto!important;
        align-items:center!important; gap:20px!important; padding:0 22px!important;
        border:0!important; border-bottom:1px solid #E5E7EB!important; border-radius:0!important;
        background:rgba(255,255,255,.985)!important; box-shadow:0 10px 26px rgba(15,23,42,.045)!important;
        backdrop-filter:blur(14px)!important; box-sizing:border-box!important;
    }}
    .cadivor-brand {{ display:flex; align-items:center; gap:12px; min-width:0; }}
    .cadivor-logo-mark {{ width:40px; height:40px; border-radius:12px; background:#2563EB; color:#fff!important; display:flex; align-items:center; justify-content:center; font-weight:950; font-size:20px; box-shadow:0 12px 22px rgba(37,99,235,.22); }}
    .cadivor-logo-text {{ color:#0F172A!important; font-size:21px; font-weight:950; line-height:1; letter-spacing:-.035em; }}
    .cadivor-logo-subtitle {{ color:#64748B!important; font-size:9.5px; font-weight:850; margin-top:4px; letter-spacing:.12em; text-transform:uppercase; }}
    .cadivor-topbar-center {{ display:flex; align-items:center; gap:14px; min-width:0; color:#0F172A!important; }}
    .cadivor-current-page {{ font-size:15px; font-weight:950; min-width:128px; }}
    .cadivor-search-pill {{ height:38px; min-width:260px; max-width:520px; flex:1 1 360px; display:flex; align-items:center; padding:0 15px; border:1px solid #E5E7EB; background:#F8FAFC; color:#94A3B8!important; border-radius:999px; font-size:12px; font-weight:800; box-shadow:inset 0 1px 0 rgba(255,255,255,.7); text-decoration:none!important; }}
    .cadivor-top-icon {{ width:34px; height:34px; border-radius:999px; display:flex; align-items:center; justify-content:center; border:1px solid #E5E7EB; background:#FFFFFF; color:#64748B!important; font-size:12px; font-weight:950; text-decoration:none!important; }}
    ...
    .cadivor-user {{ cursor:pointer; display:flex; align-items:center; gap:12px; justify-content:flex-end; min-width:260px; }}
    ...
    .cadivor-user-name {{ color:#0F172A!important; font-size:14px; font-weight:900; text-align:right; line-height:1.15; }}
    .cadivor-user-company {{ color:#64748B!important; font-size:11px; font-weight:700; text-align:right; margin-top:2px; }}
    .cadivor-avatar {{ width:40px; height:40px; border-radius:50%; display:flex; align-items:center; justify-content:center; background:#EFF6FF; color:#2563EB!important; font-weight:950; border:1px solid #BFDBFE; overflow:hidden; }}
    .cadivor-user-menu {{ position:absolute; right:0; top:100%; min-width:248px; background:#FFFFFF; border:1px solid #E5E7EB; border-radius:16px; box-shadow:0 24px 52px rgba(15,23,42,.14); padding:8px; opacity:0; transform:translateY(-4px); pointer-events:none; transition:opacity .14s ease, transform .14s ease; z-index:1000006; }}
    .cadivor-user-menu a {{ display:flex; align-items:center; justify-content:space-between; gap:12px; padding:10px 12px; border-radius:11px; text-decoration:none!important; color:#0F172A!important; font-size:13px; font-weight:800; }}
    ...
    .cv-app-sidebar {{
        ...
        z-index:1000004!important; box-sizing:border-box; background:#FFFFFF; border-right:1px solid #E5E7EB;
        padding:22px 16px; box-shadow:14px 0 34px rgba(15,23,42,.035); overflow:auto;
    }}
    .cv-side-brand {{ display:none!important; }}
    .cv-side-section {{ color:#94A3B8!important; font-size:10.5px; font-weight:900; text-transform:uppercase; letter-spacing:.09em; margin:20px 8px 10px; }}
    .cv-side-section.first {{ margin-top:0; }}
    .cv-side-nav {{ display:flex; flex-direction:column; gap:4px; }}
    .cv-side-link {{ display:flex; align-items:center; gap:12px; padding:11px 12px; border-radius:13px; color:#334155!important; text-decoration:none!important; font-size:13.2px; font-weight:800; border:1px solid transparent; }}
    .cv-side-link span {{ width:23px; text-align:center; color:#64748B!important; font-size:16px; }}
    .cv-side-link:hover {{ background:#F8FAFC; color:#0F172A!important; transform:translateX(3px); transition:all .16s ease; }}
    .cv-side-link.active {{ background:#EFF6FF; border-color:#BFDBFE; color:#2563EB!important; box-shadow:0 10px 20px rgba(37,99,235,.07); }}
    ...
    .cv-side-footer {{ margin-top:18px; display:grid; gap:8px; }}
    ...
    /* Page/dashboard spacing polish. */
    .cv-command-hero, .cv-panel, .card, .kpi-card, .brc-card {{ margin-top:0!important; }}
    .cv-section-spacer {{ margin-top:24px!important; }}
    .cv-dashboard-header {{ margin-top:0!important; margin-bottom:14px!important; }}
    .cv-title {{ color:#0F172A!important; }}
    ...
    <div id="cadivor-shell-root">
      <div id="cadivor-topbar-root" class="cadivor-topbar">
        <div class="cadivor-brand">
          ...
          <div><div class="cadivor-logo-text">Cadivor</div><div class="cadivor-logo-subtitle">Engineering Intelligence</div></div>
        </div>
        <div class="cadivor-topbar-center">
          <div class="cadivor-current-page">{app_mode}</div>
          <a class="cadivor-search-pill" href="?page=Dashboard&focus=search" target="_self">Search BOMs, parts, suppliers…</a>
          <a class="cadivor-top-icon" href="?page=Notifications" target="_self" title="Notifications">●</a>
          <a class="cadivor-top-icon" href="?page=Help" target="_self" title="Help">?</a>
        </div>
        <div class="cadivor-user-wrap">
          <div class="cadivor-user">
            <div class="cadivor-user-meta">
              <div class="cadivor-user-label">Workspace</div>
              <div class="cadivor-user-name">{shell_name}</div>
              <div class="cadivor-user-company">{shell_company}</div>
            </div>
            <div class="cadivor-avatar">{_topbar_avatar}</div>
          </div>
          <div class="cadivor-user-menu">
            <div class="cadivor-user-menu-meta"><strong>{shell_name}</strong><span>{shell_email}</span></div>
...
# Compact dashboard/action polish that does not affect the shell layout.
st.markdown(
    """
    <style>
