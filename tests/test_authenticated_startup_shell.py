...
class AuthGateStartupContracts(unittest.TestCase):
    ...
    def test_runtime_keeps_auth_gate_until_route_surface_is_ready(self):
        ...
        handoff_block = source[
            shell_mount : source.index("log_startup_phase(\"authenticated_runtime_begin\")", shell_mount)
        ]

        self.assertNotIn("retire_auth_gate_overlays()", handoff_block)
        self.assertIn("begin_authenticated_page(", source)
