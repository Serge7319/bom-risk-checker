...
def _auth_mode_label_for_root(state: str) -> str:
    return AUTH_MODE_SIGNUP if str(state) == APP_SIGNUP else AUTH_MODE_LOGIN


def _seed_auth_mode_widget(label: str) -> None:
    """Set the mode radio session key only before the widget is instantiated."""
    desired = AUTH_MODE_SIGNUP if label == AUTH_MODE_SIGNUP else AUTH_MODE_LOGIN
    st.session_state[AUTH_MODE_WIDGET_KEY] = desired


def _ensure_auth_mode_widget_seeded(initial_mode: str) -> None:
    """Seed missing radio state from root/intent without overwriting user selection."""
    current = st.session_state.get(AUTH_MODE_WIDGET_KEY)
    if current in AUTH_MODE_OPTIONS:
        return
    _seed_auth_mode_widget(initial_mode)


def _sync_root_state_from_auth_mode(auth_mode: str) -> None:
    """Mirror the outside-form radio into cadivor_root_state (no widget mutation)."""
    desired = APP_SIGNUP if auth_mode == AUTH_MODE_SIGNUP else APP_LOGIN
...
def _auth_css():
    st.markdown(
        ...
        /* Auth card is pinned to the viewport so stale/zero-height Streamlit
           slots cannot push the active credential surface below the fold. */
        ...
        .st-key-cadivor_auth_card{
            ...
            margin:0!important;
            padding:clamp(22px,3vh,34px) 34px 28px 34px!important;
            ...
            max-height:calc(100dvh - clamp(20px,4vh,44px))!important;
        ...
        .main .block-container:has(.st-key-cadivor_auth_card){
        ...
        .st-key-cadivor_auth_card > div,
        .st-key-cadivor_auth_card [data-testid="stVerticalBlock"],
        ...
        @media(max-height:900px){
            ...
            .auth-card-logo{width:44px!important;height:44px!important;margin-bottom:8px!important;font-size:20px!important;}
            .auth-card-title{font-size:23px!important;margin-bottom:3px!important;}
            .auth-copy{margin-bottom:10px!important;line-height:1.45!important;}
            .auth-strip{padding:9px 11px!important;margin-bottom:10px!important;line-height:1.35!important;}
            .auth-divider{margin:12px -30px 14px -30px!important;}
            .st-key-cadivor_auth_card div[data-testid="stRadio"]{padding:8px 11px 5px 11px!important;margin-bottom:10px!important;}
            .st-key-cadivor_auth_card div[data-testid="stTextInput"]{margin-bottom:8px!important;}
            .st-key-cadivor_auth_card .cadivor-back-home{margin-top:8px!important;}
        }
        @media(max-width:700px){
            .st-key-cadivor_auth_card{top:8px!important;width:calc(100vw - 16px)!important;max-width:calc(100vw - 16px)!important;max-height:calc(100dvh - 16px)!important;padding:20px 18px!important;border-radius:18px!important;}
            .auth-divider{margin-left:-18px!important;margin-right:-18px!important;}
        }
...
def _exit_signup_pending_to_create_account() -> None:
    ...
    st.session_state["cadivor_auth_intent_applied"] = True
    _seed_auth_mode_widget(AUTH_MODE_SIGNUP)
...
def _render_auth_card_brand(*, context_sub: str, eyebrow: str = "") -> None:
    eyebrow_markup = (
...
def _render_password_reset_request(supabase) -> None:
    ...
    if submit:
        ...
        st.rerun()
    if st.button("Back to login", key="cadivor_back_to_login_from_reset", type="secondary"):
...
def _render_password_recovery_form(supabase, cookie_manager) -> None:
    ...
    st.markdown(
        ...
        <p class="auth-copy">Your recovery link is active. Choose a secure new password to restore access to your Cadivor workspace.</p>
        <div class="auth-divider"></div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '<p class="auth-trust-note">For your security, use a password you do not use elsewhere.</p>',
        unsafe_allow_html=True,
    )
    if submit:
...
def _render_auth_page(supabase, cookie_manager, initial_mode: str):
    ...
    _ensure_auth_mode_widget_seeded(initial_mode)
    auth_mode = st.radio(
        "Choose an option",
        list(AUTH_MODE_OPTIONS),
        key=AUTH_MODE_WIDGET_KEY,
        horizontal=True,
    )
...
def show_auth_ui(supabase, cookie_manager=None):
    ...
    try:
        ...
        log_auth_correlation(
            "before_show_auth_ui",
            cookie_manager=cookie_manager,
            transition_reason="show_auth_ui_entry",
        )
    except Exception:
        pass
    ...
    if state == APP_PUBLIC and requested_auth in {"login", "signup"}:
        ...
        st.session_state["cadivor_auth_intent_applied"] = True
        # Seed only when the widget key is absent so later Create Account
        # selections are not overwritten by a lingering ?auth= query.
