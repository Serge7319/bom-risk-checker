...
def _system_instruction(*, detailed: bool = False, datasheet_qa: bool = False) -> str:
    if datasheet_qa:
        return (
            ...
            "document identifies an NPN transistor, or asking for supply voltage / VCC when "
            "the transistor datasheet only lists collector-emitter voltage VCEO). Explain the "
            "mismatch using the cited excerpts; for supply-voltage questions on transistors, "
            "state that the datasheet does not specify a supply-voltage rating and report "
            "VCEO separately when present. Do not reply with only 'Not found' when the "
            "document clearly identifies a different device type or rating concept.\n"
            "Do not invent ratings, pinouts, package details, safety approvals, or "
            "application suitability beyond the excerpts. "
            "Never present VCEO/VCES as a generic supply voltage. "
