...
def _home() -> None:
    _nav("home")
    _html(f"""
    <main class="mk-shell mk-launch-story cv634-story">
      <section class="cv634-hero">
        <div class="mk-wide cv634-hero-grid">
          <div class="cv634-copy">
            <div class="mk-eyebrow"><i class="mk-eyebrow-dot"></i>Engineering Decision Intelligence for hardware teams</div>
            <h1><span class="cv634-title-line cv634-title-main">Turn BOM risk into defensible</span><span class="cv634-title-line cv634-title-accent">engineering decisions.</span></h1>
            <p>Cadivor connects component risk, supplier evidence, AI-assisted review, alternate qualification, and decision history in one release workflow.</p>
            <div class="mk-actions"><button type="button" class="mk-btn mk-btn-primary" data-cv-auth="signup">Start 14-Day Free Trial</button><a class="mk-btn mk-btn-ghost" href="#product-workflow">Watch the Workflow</a></div>
            <div class="cv634-proof"><span>✓ No credit card</span><span>✓ CSV and Excel BOMs</span><span>✓ Evidence-linked recommendations</span></div><div class="cv635-telemetry"><span><i></i>Analyzing live product data</span><b>12-second guided review</b></div>
          </div>
          <div class="cv634-product" aria-label="Animated Cadivor BOM-to-decision product demonstration">
            <div class="cv634-windowbar"><span class="cv634-dots"><i></i><i></i><i></i></span><b>Cadivor · Motor Controller Rev C</b><em><i></i>Live review</em></div>
            <div class="cv634-app">
              <aside><strong>C</strong><span class="active">Overview</span><span>Analyze</span><span>Copilot</span><span>Alternates</span><span>Decisions</span><span>Monitor</span></aside>
              <div class="cv634-canvas">
                <div class="cv634-stage-label"><span>Release review</span><b>Motor Controller Rev C</b><em class="cv634-posture">Analyzing</em></div>
                <div class="cv634-progress"><i></i><span>Upload</span><span>Analyze</span><span>Copilot</span><span>Decide</span><span>Monitor</span></div>
                <div class="cv634-file"><div>{_icon('bom')}</div><span><b>motor_controller_rev_c.xlsx</b><small>1,842 components · 36 suppliers</small></span><em>Imported</em></div><div class="cv635-status-rail"><span class="s1">Parsing rows</span><span class="s2">Matching lifecycle</span><span class="s3">Checking suppliers</span><span class="s4">Retrieving decisions</span><span class="s5">Monitoring active</span></div>
                <div class="cv634-kpis"><article><small>BOM Health</small><strong class="cv634-health">—</strong><span>Calculating</span></article><article><small>Release Blockers</small><strong class="cv634-blockers">—</strong><span>Scanning</span></article><article><small>Lifecycle Alerts</small><strong class="cv634-alerts">—</strong><span>Checking</span></article><article><small>Qualified Alternates</small><strong class="cv634-alts">—</strong><span>Matching</span></article></div>
                <div class="cv634-workarea">
                  <div class="cv634-risks"><header><b>Priority risks</b><span>Ranked by production impact</span></header><div class="cv634-risk r1"><i>1</i><span><b>MPU6050</b><small>EOL · single approved source</small></span><em>Blocker</em></div><div class="cv634-risk r2"><i>2</i><span><b>LM35DN</b><small>18-week lead time · source concentration</small></span><em>Qualify</em></div><div class="cv634-risk r3"><i>3</i><span><b>TPS54331</b><small>Lifecycle change detected</small></span><em>Monitor</em></div></div>
                  <div class="cv634-insight"><header><span>{_icon('chat')} Cadivor Copilot</span><em>93% confidence</em></header><p class="cv634-question">Is this BOM ready for production?</p><div class="cv634-answer"><b>No. Two issues block release.</b><span>Replace MPU6050 and qualify the LM35DN second source before approval.</span><small>14 evidence points · lifecycle · supplier · decision history</small></div><div class="cv634-decision"><span>Decision DR-1048</span><b>Mitigation approved</b><em>Monitoring active</em></div></div>
                </div>
              </div>
            </div>
            <div class="cv634-live-events"><span class="e1"><b>Lifecycle evidence</b> EOL notice matched</span><span class="e2"><b>Supply evidence</b> Second source found</span><span class="e3"><b>Decision evidence</b> DR-1048 approved</span></div><div class="cv634-demo-caption"><span class="s1">BOM imported</span><span class="s2">1,842 components analyzed</span><span class="s3">Two release blockers found</span><span class="s4">14 evidence points retrieved</span><span class="s5">Alternate qualification proposed</span><span class="s6">Decision approved · monitoring active</span></div>
          </div>
        </div>
      </section>

      <section class="cv634-coverage"><div class="mk-wrap"><span>Authorized supplier coverage</span><b>DigiKey</b><b>Mouser</b><b>Newark</b><i></i><b>Octopart <small>Coming Soon</small></b></div></section>

      <section class="cv634-section cv634-workflow" id="product-workflow"><div class="mk-wrap"><div class="cv634-heading"><span>One connected workflow</span><h2>Follow the evidence from uploaded BOM to approved action.</h2><p>The same engineering context moves through every stage. No disconnected dashboards, copied spreadsheets, or lost rationale.</p></div><div class="cv634-pipeline"><div class="cv634-flowline"><i></i></div><article><em>01</em><div>{_icon('bom')}</div><span><b>Upload BOM</b><small>motor_controller_rev_c.xlsx</small></span><strong>1,842 parts</strong></article><article><em>02</em><div>{_icon('portfolio')}</div><span><b>Analyze Risk</b><small>Lifecycle, supply, cost, alternatives</small></span><strong>4 high risk</strong></article><article><em>03</em><div>{_icon('chat')}</div><span><b>AI Copilot</b><small>Evidence-grounded release review</small></span><strong>93% confidence</strong></article><article><em>04</em><div>{_icon('decision')}</div><span><b>Engineering Decision</b><small>Owner, rationale, evidence, history</small></span><strong>DR-1048 approved</strong></article><article><em>05</em><div>{_icon('monitor')}</div><span><b>Continuous Monitoring</b><small>Lifecycle, stock, supplier, and PCN changes</small></span><strong>4,826 parts watched</strong></article></div></div></section>

      <section class="cv634-section cv634-command cv635-dark-stage" id="engineering-intelligence"><div class="mk-wide"><div class="cv634-heading light"><span>Engineering Intelligence</span><h2>A release command center, not another alert dashboard.</h2><p>Cadivor ranks what can actually stop production and shows the evidence, owner, and next action beside every issue.</p></div><div class="cv634-command-grid"><div class="cv634-command-main"><div class="cv634-command-top"><div><small>PROGRAM HEALTH</small><h3>Motor Controller Platform</h3></div><span>3 products · 4,826 monitored parts</span></div><div class="cv634-command-metrics"><article><small>Portfolio Health</small><b>84</b><em>+6 this quarter</em></article><article><small>Release Blockers</small><b>2</b><em>Both assigned</em></article><article><small>Exposure Avoided</small><b>$184K</b><em>Estimated annual impact</em></article></div><div class="cv634-exposure"><header><b>Release exposure by product</b><span>Current engineering posture</span></header><div><span>Motor Controller Rev C</span><i><em style="width:72%"></em></i><b>Hold</b></div><div><span>Sensor Hub Rev B</span><i><em style="width:91%"></em></i><b class="green">Ready</b></div><div><span>Power Board Rev A</span><i><em style="width:82%"></em></i><b>Review</b></div></div></div><div class="cv634-action-rail"><header>Action queue <span>6 open</span></header><article><em>BLOCKER</em><b>Replace MPU6050</b><p>EOL and single-source exposure</p><small>Owner · J. Patel · Due Aug 5</small></article><article><em class="amber">QUALIFY</em><b>Approve LM35DN alternate</b><p>Second-source validation required</p><small>Owner · M. Chen · Due Aug 8</small></article><article><em class="blue">MONITOR</em><b>Track TPS54331 PCN</b><p>Lifecycle change under review</p><small>Owner · Cadivor · Continuous</small></article><button type="button" data-cv-public="product">Explore Engineering Intelligence →</button></div></div></div></section>

      <section class="cv634-section cv634-copilot cv635-light-stage" id="copilot"><div class="mk-wide cv634-split"><div class="cv634-copy-block"><span>AI Engineering Copilot</span><h2>See the evidence before accepting the answer.</h2><p>Cadivor does not hide behind a chat response. It retrieves the active BOM, supplier records, lifecycle notices, alternatives, and prior decisions before recommending an action.</p><ul><li>Context-aware follow-up questions</li><li>Visible confidence and evidence sources</li><li>Clear conditions that would change the recommendation</li></ul><button type="button" class="mk-btn mk-btn-light" data-cv-public="product">Explore the Copilot</button></div><div class="cv634-copilot-ui"><div class="cv634-copilot-head"><span>{_icon('chat')} Engineering Copilot</span><em>Motor Controller Rev C</em></div><div class="cv634-user"><small>You</small><p>Is this BOM ready for production release?</p></div><div class="cv634-retrieval"><span>Retrieving evidence</span><i></i><b>BOM-1048</b><b>1,842 component records</b><b>3 supplier feeds</b><b>12 prior decisions</b></div><div class="cv634-ai"><small>Cadivor · 93% confidence</small><h3>Production release is blocked.</h3><p>MPU6050 is end-of-life with one approved source. LM35DN also requires a qualified second source before release.</p><div><span><b>01</b>Replace MPU6050</span><em>Release blocker</em></div><div><span><b>02</b>Qualify LM35DN alternate</span><em>Required action</em></div><footer><span>14 evidence points</span><span>What would change this?</span></footer></div></div></div></section>

      <section class="cv634-section cv634-supplier cv635-soft-stage" id="supplier-intelligence"><div class="mk-wide"><div class="cv634-heading"><span>Supplier Intelligence</span><h2>Evaluate sourcing resilience across the full component context.</h2><p>Inventory alone does not make a part safe. Compare authorization, lead time, lifecycle, geographic concentration, and qualification status in one workspace.</p></div><div class="cv634-supplier-ui"><div class="cv634-supplier-head"><div><small>COMPONENT REVIEW</small><h3>LM35DN · Temperature Sensor</h3></div><span>Second source required</span></div><div class="cv634-supplier-kpis"><article><small>Authorized Sources</small><b>2</b><em>1 approved</em></article><article><small>Total Inventory</small><b>38,420</b><em>Across 3 regions</em></article><article><small>Lead Time</small><b>18 weeks</b><em>Above threshold</em></article><article><small>Concentration Risk</small><b>High</b><em>74% one supplier</em></article></div><div class="cv634-supplier-table"><header><span>Supplier</span><span>Authorization</span><span>Inventory</span><span>Lead time</span><span>Region</span><span>Qualification</span></header><div class="selected"><b>DigiKey</b><span>Authorized</span><span>14,200</span><span>18 weeks</span><span>North America</span><em>Approved</em></div><div><b>Mouser</b><span>Authorized</span><span>9,840</span><span>16 weeks</span><span>North America</span><em class="amber">Qualify</em></div><div><b>Newark</b><span>Authorized</span><span>6,380</span><span>20 weeks</span><span>Europe / US</span><em>Available</em></div></div><div class="cv634-supplier-action"><div><span>Cadivor recommendation</span><b>Qualify Mouser as the approved second source.</b><p>This reduces supplier concentration while preserving the existing manufacturer part number and engineering specification.</p></div><strong>91% confidence</strong></div></div></div></section>

      <section class="cv634-section cv634-decision cv635-paper-stage" id="decision-intelligence"><div class="mk-wide cv634-decision-grid"><div class="cv634-decision-copy"><span>Decision Intelligence</span><h2>The final product is not an alert. It is a defensible decision.</h2><p>Preserve what was decided, who approved it, the evidence reviewed, and what Cadivor should monitor next.</p><div><b>✓</b><span><strong>Traceable</strong> Every recommendation links to its supporting evidence.</span></div><div><b>✓</b><span><strong>Accountable</strong> Owners, dates, approvals, and open conditions remain visible.</span></div><div><b>✓</b><span><strong>Persistent</strong> Decisions remain part of the product’s engineering history.</span></div></div><div class="cv634-record"><header><div><small>ENGINEERING DECISION RECORD</small><h3>DR-1048 · Motor Controller Rev C</h3></div><span>APPROVED</span></header><div class="cv634-record-meta"><div><small>Decision owner</small><b>J. Patel · Hardware Lead</b></div><div><small>Approved</small><b>August 1, 2026 · 09:42</b></div><div><small>AI confidence</small><b>93% · 14 evidence points</b></div></div><section><small>DECISION</small><h4>Replace MPU6050 and qualify LM35DN second source before release.</h4><p>The current design cannot proceed to production until both lifecycle and supplier-concentration risks are mitigated.</p></section><div class="cv634-rationale"><small>RATIONALE</small><p>MPU6050 is end-of-life with no approved alternate. LM35DN remains viable, but its approved-source concentration exceeds the team’s production threshold.</p></div><footer><div><span>09:31</span><b>Cadivor recommendation generated</b></div><div><span>09:36</span><b>Engineering evidence reviewed</b></div><div><span>09:42</span><b>Decision approved · monitoring activated</b></div></footer></div></div></section>

      <section class="cv634-final"><div class="mk-wrap"><span>Bring one real BOM.</span><h2>See how Cadivor turns it into a release decision.</h2><p>Start a 14-day full-access trial or book a focused walkthrough using a representative BOM.</p><div class="mk-actions"><button type="button" class="mk-btn mk-btn-primary" data-cv-auth="signup">Start Free Trial</button><button type="button" class="mk-btn mk-btn-light" data-cv-public="contact">Book a BOM Review</button></div></div></section>
    </main>
    """)
    _footer("home")
...
def _product() -> None:
    ...
    <section class="mk-section cv6351-product-copilot" id="copilot"><div class="mk-wrap mk-feature-split"><div class="mk-surface cv6351-ask-demo"><div class="mk-cockpit"><div class="mk-cockpit-title"><span>Ask Cadivor</span><em><i></i>Analyzing active BOM</em></div><div class="cv6351-chat-user"><small>Engineer</small><strong><span class="cv6352-typed-question">Is this BOM ready for production release?</span><i class="cv6352-caret"></i></strong></div><div class="cv6351-thinking"><span></span><span></span><span></span><b class="cv6352-thinking-label">Reviewing engineering evidence</b></div><div class="cv6351-evidence"><span class="e1">BOM-1048 loaded</span><span class="e2">Lifecycle notices checked</span><span class="e3">Supplier exposure compared</span><span class="e4">Qualified alternates reviewed</span></div><div class="cv6351-chat-ai"><small>Cadivor · 93% confidence</small><strong><span class="cv6352-typed-answer">Production release is blocked.</span><i class="cv6352-ai-caret"></i></strong><p>Two components create unresolved lifecycle and single-source exposure.</p><div><b>01</b><span>Replace MPU6050 before release</span><em>Blocker</em></div><div><b>02</b><span>Qualify a second source for LM35DN</span><em>Required</em></div></div><div class="cv6351-followup"><span>Suggested follow-up</span><b>What evidence would change this recommendation?</b></div></div></div><div class="mk-feature-copy"><div class="mk-kicker">Ask Cadivor</div><h2>Challenge the analysis in plain language.</h2><p>Watch Cadivor retrieve BOM, lifecycle, supplier, and alternate-part evidence before reaching an engineering conclusion.</p><div class="cv6351-capabilities"><span>Evidence retrieval</span><span>Confidence scoring</span><span>Actionable conclusions</span></div></div></div></section>
...
def _contact() -> None:
    ...
    _html(f"""
    <section class="mk-section soft"><div class="mk-wrap"><div class="mk-card-grid"><div class="mk-card"><div class="mk-icon">▶</div><h3>Book a product demo</h3><p>Walk through BOM analysis, Engineering Intelligence, Ask Cadivor, alternatives, monitoring, and reports using a representative workflow.</p><a href="mailto:{GENERAL_EMAIL}?subject=Cadivor%20Demo%20Request">{GENERAL_EMAIL} →</a></div><div class="mk-card"><div class="mk-icon">β</div><h3>Join the beta program</h3><p>Evaluate Cadivor with a real engineering team and provide feedback that directly influences launch priorities.</p><a href="mailto:{BETA_EMAIL}?subject=Cadivor%20Beta%20Program">Request beta access →</a></div><div class="mk-card"><div class="mk-icon">?</div><h3>Product and support</h3><p>Ask a question about accounts, BOM formatting, plan entitlements, reports, or the engineering workflow.</p><a href="mailto:{SUPPORT_EMAIL}?subject=Cadivor%20Product%20Question">Contact support →</a></div></div></div></section>
...
def _legal_page(kind: str) -> None:
    ...
    if kind == "security":
        ...
        <h2>10. Contact</h2>
        <p>Questions about these Terms may be sent to <a href="mailto:{LEGAL_EMAIL}?subject=Cadivor%20Terms%20Inquiry"><strong>{LEGAL_EMAIL}</strong></a>.</p>
...
def _sprint_63_4_product_led_css() -> None:
    st.markdown("""
    <style>
    .cv634-story{--c634-navy:#07162f;--c634-blue:#2f6df6;--c634-line:#dce5f0;--c634-soft:#f5f8fc;--c634-green:#169b62;--c634-red:#dc4048;--c634-amber:#d98916}
    .cv634-story *{box-sizing:border-box}.cv634-hero{position:relative;padding:74px 0 82px;background:radial-gradient(circle at 83% 8%,rgba(65,126,245,.19),transparent 31%),linear-gradient(145deg,#06142c 0%,#091b39 60%,#0d2853 100%);color:#fff;overflow:hidden}.cv634-hero-grid{display:grid;grid-template-columns:minmax(520px,.92fr) minmax(680px,1.28fr);gap:44px;align-items:center}.cv634-copy h1{max-width:760px;margin:22px 0 22px;color:#fff!important;font-size:clamp(44px,4.15vw,64px);line-height:1.01;letter-spacing:-.052em}.cv634-title-line{display:block;white-space:nowrap}.cv634-title-main{color:#fff}.cv634-title-accent{color:#78a9ff}.cv634-copy>p{max-width:650px;margin:0;color:#c9d7eb!important;font-size:18px;line-height:1.7}.cv634-copy .mk-actions{margin:31px 0 20px}.cv634-copy .mk-btn-ghost{border-color:rgba(255,255,255,.28)!important;color:#fff!important}.cv634-proof{display:flex;flex-wrap:wrap;gap:9px 18px;color:#aebed4;font-size:12px;font-weight:700}.cv634-product{position:relative;min-width:0;border:1px solid rgba(143,180,242,.32);border-radius:20px;background:#fff;box-shadow:0 40px 100px rgba(0,0,0,.36);overflow:hidden;transform:perspective(1500px) rotateY(-1.6deg)}.cv634-windowbar{height:48px;padding:0 16px;display:flex;align-items:center;gap:14px;background:#edf2f8;border-bottom:1px solid #d7e0eb;color:#526379;font-size:10px}.cv634-windowbar>b{flex:1}.cv634-windowbar em{display:flex;align-items:center;gap:7px;color:#2463d4;font-style:normal;font-weight:850}.cv634-windowbar em i{width:7px;height:7px;border-radius:50%;background:#1dad6b;animation:cv634-pulse 1.8s infinite}.cv634-dots{display:flex;gap:5px}.cv634-dots i{width:8px;height:8px;border-radius:50%;background:#c8d2df}.cv634-dots i:first-child{background:#f06b68}.cv634-dots i:nth-child(2){background:#efbc53}.cv634-dots i:nth-child(3){background:#59bf7b}.cv634-app{display:grid;grid-template-columns:88px 1fr;min-height:550px}.cv634-app aside{padding:16px 9px;background:#081a37;color:#9fb0c8;display:flex;flex-direction:column;gap:8px}.cv634-app aside strong{width:32px;height:32px;margin:0 auto 9px;border-radius:9px;background:#2f6df6;display:grid;place-items:center;color:#fff;font-size:17px}.cv634-app aside span{padding:8px 5px;border-radius:7px;text-align:center;font-size:8px;font-weight:750}.cv634-app aside span.active{background:#173867;color:#fff}.cv634-canvas{padding:19px;background:#f6f8fb;min-width:0}.cv634-stage-label{display:flex;align-items:center;gap:12px}.cv634-stage-label>span{font-size:8px;letter-spacing:.1em;color:#7a899c;font-weight:850;text-transform:uppercase}.cv634-stage-label>b{flex:1;color:#10213d;font-size:15px}.cv634-stage-label em{padding:6px 9px;border-radius:999px;background:#fff3db;color:#b26a08;font-size:8px;font-style:normal;font-weight:900}.cv634-progress{position:relative;display:grid;grid-template-columns:repeat(5,1fr);gap:0;margin:16px 0 13px;padding-top:9px}.cv634-progress::before{content:"";position:absolute;left:7%;right:7%;top:2px;height:3px;border-radius:999px;background:#dce4ee}.cv634-progress i{position:absolute;left:7%;top:2px;height:3px;width:0;border-radius:999px;background:#2f6df6;animation:cv634-progress 12s ease-in-out infinite}.cv634-progress span{text-align:center;color:#7a899c;font-size:7px;font-weight:800}.cv634-file{display:flex;align-items:center;gap:10px;padding:10px 12px;border:1px solid #dae3ee;border-radius:10px;background:#fff}.cv634-file>div{width:29px;height:29px;border-radius:8px;background:#e8f0ff;display:grid;place-items:center;color:#2f6df6}.cv634-file svg,.cv634-copilot-head svg,.cv634-insight header svg,.cv634-pipeline svg{width:16px;height:16px;fill:none;stroke:currentColor;stroke-width:1.8}.cv634-file span{display:flex;flex:1;flex-direction:column}.cv634-file b{font-size:9px;color:#172a47}.cv634-file small{font-size:7px;color:#7a899c}.cv634-file em{font-size:7px;font-style:normal;font-weight:850;color:#169b62}.cv634-kpis{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:10px 0}.cv634-kpis article{padding:10px;border:1px solid #dce4ee;border-radius:9px;background:#fff}.cv634-kpis small{display:block;color:#718096;font-size:7px}.cv634-kpis strong{display:block;margin:2px 0;color:#0b1c37;font-size:18px}.cv634-health,.cv634-blockers,.cv634-alerts,.cv634-alts{font-size:0!important}.cv634-health::after,.cv634-blockers::after,.cv634-alerts::after,.cv634-alts::after{font-size:18px}.cv634-health::after{content:"—";animation:cv634-health 12s infinite}.cv634-blockers::after{content:"—";animation:cv634-blockers 12s infinite}.cv634-alerts::after{content:"—";animation:cv634-alerts 12s infinite}.cv634-alts::after{content:"—";animation:cv634-alts 12s infinite}.cv634-posture{animation:cv634-posture 12s infinite}.cv634-kpis span{font-size:7px;color:#8190a3}.cv634-workarea{display:grid;grid-template-columns:1.08fr .92fr;gap:10px}.cv634-risks,.cv634-insight{border:1px solid #dce4ee;border-radius:10px;background:#fff;overflow:hidden}.cv634-risks header,.cv634-insight header{padding:10px 11px;border-bottom:1px solid #e4eaf1;display:flex;justify-content:space-between;align-items:center}.cv634-risks header b,.cv634-insight header span{color:#17304f;font-size:9px;font-weight:850}.cv634-risks header span,.cv634-insight header em{font-size:7px;color:#78879a;font-style:normal}.cv634-risk{display:grid;grid-template-columns:22px 1fr auto;gap:8px;align-items:center;padding:9px 11px;border-bottom:1px solid #edf1f5;opacity:.2;transform:translateX(-8px)}.cv634-risk i{width:20px;height:20px;border-radius:6px;display:grid;place-items:center;background:#f2f5f9;color:#53657a;font-size:7px;font-style:normal;font-weight:900}.cv634-risk span{display:flex;flex-direction:column}.cv634-risk b{font-size:8px;color:#162942}.cv634-risk small{font-size:7px;color:#77869a}.cv634-risk em{font-size:7px;font-style:normal;font-weight:850;color:#c13a41}.cv634-risk.r1{animation:cv634-risk-in 12s 2.1s infinite}.cv634-risk.r2{animation:cv634-risk-in 12s 2.7s infinite}.cv634-risk.r3{animation:cv634-risk-in 12s 3.3s infinite}.cv634-question{margin:10px!important;padding:8px 9px;border-radius:8px;background:#edf3fc;color:#294769!important;font-size:8px!important}.cv634-answer{margin:8px 10px;padding:10px;border-radius:8px;background:#0d2447;color:#fff;opacity:0;animation:cv634-answer 12s 4.8s infinite}.cv634-answer b,.cv634-answer span,.cv634-answer small{display:block}.cv634-answer b{font-size:9px}.cv634-answer span{margin:4px 0;color:#dbe8f8;font-size:7px;line-height:1.45}.cv634-answer small{color:#8fb5ee;font-size:6px}.cv634-decision{margin:8px 10px;padding:9px;border:1px solid #cce8da;border-radius:8px;background:#effaf5;display:flex;flex-direction:column;opacity:0;animation:cv634-decision-in 12s 7.7s infinite}.cv634-decision span{font-size:6px;color:#678175}.cv634-decision b{font-size:8px;color:#11643f}.cv634-decision em{font-size:6px;color:#169b62;font-style:normal}.cv634-live-events{display:grid;grid-template-columns:repeat(3,1fr);gap:7px;margin-top:9px}.cv634-live-events span{padding:7px 8px;border:1px solid #dce4ee;border-radius:8px;background:#fff;color:#718096;font-size:6px;opacity:.28;transform:translateY(4px)}.cv634-live-events b{display:block;margin-bottom:2px;color:#18304d;font-size:7px}.cv634-live-events .e1{animation:cv634-event 12s 3.4s infinite}.cv634-live-events .e2{animation:cv634-event 12s 6.2s infinite}.cv634-live-events .e3{animation:cv634-event 12s 8.6s infinite}.cv634-demo-caption{position:absolute;left:50%;bottom:12px;transform:translateX(-50%);min-width:225px;height:26px;padding:0 12px;border:1px solid rgba(133,174,239,.22);border-radius:999px;background:rgba(6,20,44,.93);color:#d8e6fa;overflow:hidden}.cv634-demo-caption span{position:absolute;inset:0;display:grid;place-items:center;font-size:8px;font-weight:800;opacity:0}.cv634-demo-caption .s1{animation:cv634-caption 12s 0s infinite}.cv634-demo-caption .s2{animation:cv634-caption 12s 1.8s infinite}.cv634-demo-caption .s3{animation:cv634-caption 12s 3.6s infinite}.cv634-demo-caption .s4{animation:cv634-caption 12s 5.4s infinite}.cv634-demo-caption .s5{animation:cv634-caption 12s 7.2s infinite}.cv634-demo-caption .s6{animation:cv634-caption 12s 9s infinite}
    .cv634-coverage{border-bottom:1px solid #e0e7f0;background:#fff}.cv634-coverage .mk-wrap{min-height:74px;display:flex;align-items:center;justify-content:center;gap:38px;color:#738299}.cv634-coverage span{font-size:10px;font-weight:850;text-transform:uppercase;letter-spacing:.1em}.cv634-coverage b{color:#42536b;font-size:16px}.cv634-coverage small{color:#2f6df6;font-size:7px}.cv634-coverage i{width:1px;height:24px;background:#dbe3ed}
    .cv634-section{padding:112px 0}.cv634-heading{max-width:790px;margin:0 auto 52px;text-align:center}.cv634-heading>span,.cv634-copy-block>span,.cv634-decision-copy>span{color:#2f6df6;font-size:11px;font-weight:900;letter-spacing:.12em;text-transform:uppercase}.cv634-heading h2,.cv634-copy-block h2,.cv634-decision-copy h2{margin:12px 0 14px;color:#0b1730!important;font-size:clamp(36px,4vw,57px);line-height:1.07;letter-spacing:-.045em}.cv634-heading p,.cv634-copy-block>p,.cv634-decision-copy>p{color:#617188!important;font-size:17px;line-height:1.7}.cv634-heading.light h2,.cv634-heading.light p{color:#fff!important}.cv634-heading.light p{color:#b8c8dd!important}
    .cv634-workflow{background:#fff}.cv634-pipeline{position:relative;display:grid;grid-template-columns:repeat(4,1fr);gap:16px}.cv634-flowline{position:absolute;left:8%;right:8%;top:50%;height:3px;background:#dce5f0}.cv634-flowline i{display:block;width:22%;height:100%;background:linear-gradient(90deg,#2f6df6,#72a4ff);animation:cv634-flow 8s ease-in-out infinite}.cv634-pipeline article{position:relative;z-index:1;min-height:230px;padding:24px;border:1px solid #dce5f0;border-radius:16px;background:#fff;box-shadow:0 18px 50px rgba(15,35,70,.07);display:flex;flex-direction:column}.cv634-pipeline article>em{color:#8da0b7;font-size:10px;font-style:normal;font-weight:900}.cv634-pipeline article>div{width:44px;height:44px;margin:24px 0 19px;border-radius:12px;background:#eaf1ff;color:#2f6df6;display:grid;place-items:center}.cv634-pipeline article span{display:flex;flex-direction:column}.cv634-pipeline article b{color:#132744;font-size:17px}.cv634-pipeline article small{margin-top:5px;color:#6e7f94;font-size:11px;line-height:1.45}.cv634-pipeline article>strong{margin-top:auto;padding-top:20px;color:#2f6df6;font-size:11px}
    .cv634-command{background:linear-gradient(145deg,#06152e,#0a2248);color:#fff}.cv634-command-grid{display:grid;grid-template-columns:1.45fr .55fr;gap:18px}.cv634-command-main,.cv634-action-rail{border:1px solid rgba(136,172,229,.22);border-radius:18px;background:rgba(255,255,255,.055);box-shadow:0 28px 75px rgba(0,0,0,.18)}.cv634-command-main{padding:28px}.cv634-command-top{display:flex;justify-content:space-between;align-items:center}.cv634-command-top small{color:#78a9ff;font-size:9px;font-weight:900;letter-spacing:.11em}.cv634-command-top h3{margin:5px 0 0;color:#fff;font-size:24px}.cv634-command-top>span{color:#9fb3cc;font-size:11px}.cv634-command-metrics{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:25px 0}.cv634-command-metrics article{padding:18px;border:1px solid rgba(145,177,226,.18);border-radius:13px;background:rgba(255,255,255,.045)}.cv634-command-metrics small,.cv634-command-metrics b,.cv634-command-metrics em{display:block}.cv634-command-metrics small{color:#98abc4;font-size:9px}.cv634-command-metrics b{margin:5px 0;color:#fff;font-size:30px}.cv634-command-metrics em{color:#78d6a7;font-size:9px;font-style:normal}.cv634-exposure{padding:18px;border-radius:13px;background:#fff;color:#17304f}.cv634-exposure header{display:flex;justify-content:space-between;margin-bottom:13px}.cv634-exposure header b{font-size:12px}.cv634-exposure header span{color:#7a899d;font-size:9px}.cv634-exposure>div{display:grid;grid-template-columns:170px 1fr 52px;gap:13px;align-items:center;padding:10px 0;border-top:1px solid #e8edf3;font-size:10px}.cv634-exposure i{height:7px;border-radius:99px;background:#e9eef5;overflow:hidden}.cv634-exposure i em{display:block;height:100%;background:linear-gradient(90deg,#efae45,#e45959)}.cv634-exposure>div>b{color:#c34449}.cv634-exposure>div>b.green{color:#169b62}.cv634-action-rail{padding:22px}.cv634-action-rail header{display:flex;justify-content:space-between;margin-bottom:13px;color:#fff;font-size:13px;font-weight:850}.cv634-action-rail header span{color:#8fa7c6;font-size:10px}.cv634-action-rail article{padding:15px 0;border-top:1px solid rgba(158,185,225,.16)}.cv634-action-rail article em{display:inline-flex;padding:4px 7px;border-radius:99px;background:rgba(220,64,72,.14);color:#ff9499;font-size:7px;font-style:normal;font-weight:900}.cv634-action-rail article em.amber{background:rgba(217,137,22,.14);color:#ffc36f}.cv634-action-rail article em.blue{background:rgba(47,109,246,.18);color:#8fb4ff}.cv634-action-rail article b{display:block;margin:8px 0 3px;color:#fff;font-size:12px}.cv634-action-rail article p{margin:0;color:#aebfd4!important;font-size:10px}.cv634-action-rail article small{color:#7890ad;font-size:8px}.cv634-action-rail button{margin-top:12px;padding:0;border:0;background:transparent;color:#83adf5;font-size:10px;font-weight:850;cursor:pointer}
    .cv634-copilot{background:#f4f7fb}.cv634-split{display:grid;grid-template-columns:.78fr 1.22fr;gap:76px;align-items:center}.cv634-copy-block ul{margin:25px 0 30px;padding:0;list-style:none}.cv634-copy-block li{padding:9px 0;color:#43556d;font-size:14px}.cv634-copy-block li::before{content:"✓";margin-right:10px;color:#169b62;font-weight:900}.cv634-copilot-ui{border:1px solid #d7e1ee;border-radius:18px;background:#fff;box-shadow:0 28px 75px rgba(20,42,78,.13);overflow:hidden}.cv634-copilot-head{height:55px;padding:0 19px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #e1e7ef}.cv634-copilot-head span{display:flex;gap:9px;align-items:center;color:#17324f;font-weight:850}.cv634-copilot-head em{padding:6px 9px;border-radius:99px;background:#edf3fc;color:#3567b3;font-size:9px;font-style:normal}.cv634-user{margin:24px 24px 12px 22%;padding:13px 15px;border-radius:13px 13px 3px 13px;background:#edf3fc}.cv634-user small{color:#718198;font-size:8px}.cv634-user p{margin:4px 0 0;color:#294766!important;font-size:13px}.cv634-retrieval{margin:12px 24px;padding:14px;border:1px solid #dce5ef;border-radius:11px;display:flex;gap:7px;flex-wrap:wrap;align-items:center}.cv634-retrieval>span{width:100%;color:#718198;font-size:8px;font-weight:850;text-transform:uppercase;letter-spacing:.08em}.cv634-retrieval i{width:100%;height:3px;border-radius:99px;background:linear-gradient(90deg,#2f6df6 62%,#e4eaf2 62%);animation:cv634-retrieve 4s ease-in-out infinite}.cv634-retrieval b{padding:5px 7px;border-radius:6px;background:#f3f6fa;color:#52657b;font-size:8px}.cv634-ai{margin:12px 24px 24px;padding:20px;border-radius:13px;background:#0b2347;color:#fff}.cv634-ai>small{color:#85b0f5;font-size:9px}.cv634-ai h3{margin:7px 0;color:#fff;font-size:21px}.cv634-ai>p{color:#d5e2f3!important;font-size:12px;line-height:1.6}.cv634-ai>div{margin-top:9px;padding:10px;border:1px solid rgba(150,182,231,.18);border-radius:9px;display:flex;justify-content:space-between;align-items:center}.cv634-ai>div span{color:#e8f1fc;font-size:10px}.cv634-ai>div span b{margin-right:8px;color:#7fb0ff}.cv634-ai>div em{font-size:8px;font-style:normal;color:#ffb0b4}.cv634-ai>div:nth-of-type(2) em{color:#ffc777}.cv634-ai footer{display:flex;justify-content:space-between;margin-top:15px;color:#8facd2;font-size:8px}
    .cv634-supplier{background:#fff}.cv634-supplier-ui{border:1px solid #d9e2ed;border-radius:18px;background:#fff;box-shadow:0 28px 78px rgba(20,42,78,.1);overflow:hidden}.cv634-supplier-head{padding:22px 25px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #e3e9f0}.cv634-supplier-head small{color:#2f6df6;font-size:8px;font-weight:900;letter-spacing:.1em}.cv634-supplier-head h3{margin:4px 0 0;color:#122744;font-size:22px}.cv634-supplier-head>span{padding:7px 10px;border-radius:99px;background:#fff3df;color:#a9650b;font-size:9px;font-weight:850}.cv634-supplier-kpis{display:grid;grid-template-columns:repeat(4,1fr);border-bottom:1px solid #e3e9f0}.cv634-supplier-kpis article{padding:19px 24px;border-right:1px solid #e3e9f0}.cv634-supplier-kpis article:last-child{border:0}.cv634-supplier-kpis small,.cv634-supplier-kpis b,.cv634-supplier-kpis em{display:block}.cv634-supplier-kpis small{color:#758499;font-size:9px}.cv634-supplier-kpis b{margin:4px 0;color:#132845;font-size:24px}.cv634-supplier-kpis em{color:#b36b0c;font-size:8px;font-style:normal}.cv634-supplier-table{padding:8px 20px}.cv634-supplier-table header,.cv634-supplier-table>div{display:grid;grid-template-columns:1.1fr 1fr .8fr .8fr 1.1fr .8fr;gap:13px;align-items:center;padding:13px 10px}.cv634-supplier-table header{color:#7b899b;font-size:8px;font-weight:850;text-transform:uppercase}.cv634-supplier-table>div{border-top:1px solid #e8edf3;color:#516277;font-size:10px}.cv634-supplier-table>div.selected{border-radius:9px;background:#f3f7fd}.cv634-supplier-table>div b{color:#17304e}.cv634-supplier-table em{justify-self:start;padding:5px 8px;border-radius:99px;background:#eaf8f1;color:#158451;font-size:8px;font-style:normal;font-weight:850}.cv634-supplier-table em.amber{background:#fff3df;color:#a9650b}.cv634-supplier-action{display:flex;align-items:center;justify-content:space-between;gap:30px;padding:22px 25px;background:#0b2347;color:#fff}.cv634-supplier-action span{color:#84aff2;font-size:8px;font-weight:850;text-transform:uppercase}.cv634-supplier-action b{display:block;margin:5px 0;color:#fff;font-size:15px}.cv634-supplier-action p{margin:0;color:#bdcde1!important;font-size:10px}.cv634-supplier-action>strong{flex:none;padding:9px 12px;border-radius:9px;background:rgba(255,255,255,.09);font-size:11px}
    .cv634-decision{background:#f4f7fb}.cv634-decision-grid{display:grid;grid-template-columns:.65fr 1.35fr;gap:70px;align-items:center}.cv634-decision-copy>div{display:flex;gap:12px;margin-top:17px;color:#42536b}.cv634-decision-copy>div>b{width:25px;height:25px;flex:none;border-radius:50%;background:#e8f7f0;display:grid;place-items:center;color:#169b62}.cv634-decision-copy>div span{font-size:12px;line-height:1.55}.cv634-decision-copy>div strong{display:block;color:#17304f}.cv634-record{border:1px solid #d7e1ec;border-top:5px solid #169b62;border-radius:17px;background:#fff;box-shadow:0 30px 80px rgba(20,42,78,.13);overflow:hidden}.cv634-record header{padding:22px 24px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #e1e8ef}.cv634-record header small{color:#718198;font-size:8px;font-weight:850;letter-spacing:.1em}.cv634-record header h3{margin:5px 0 0;color:#142944;font-size:20px}.cv634-record header>span{padding:7px 10px;border-radius:99px;background:#e9f8f0;color:#14834f;font-size:9px;font-weight:900}.cv634-record-meta{display:grid;grid-template-columns:repeat(3,1fr);border-bottom:1px solid #e1e8ef}.cv634-record-meta>div{padding:16px 20px;border-right:1px solid #e1e8ef}.cv634-record-meta>div:last-child{border:0}.cv634-record-meta small,.cv634-record-meta b{display:block}.cv634-record-meta small{color:#7b899b;font-size:8px}.cv634-record-meta b{margin-top:4px;color:#19304c;font-size:10px}.cv634-record>section{padding:22px 24px}.cv634-record>section small,.cv634-rationale small{color:#2f6df6;font-size:8px;font-weight:900;letter-spacing:.09em}.cv634-record h4{margin:7px 0;color:#142944;font-size:19px;line-height:1.35}.cv634-record>section p,.cv634-rationale p{color:#617188!important;font-size:11px;line-height:1.6}.cv634-rationale{margin:0 24px 22px;padding:15px;border-left:3px solid #2f6df6;background:#f3f7fc}.cv634-record>footer{padding:0 24px 22px}.cv634-record>footer div{display:grid;grid-template-columns:55px 1fr;gap:10px;padding:9px 0;border-top:1px solid #e8edf3}.cv634-record>footer span{color:#8390a0;font-size:8px}.cv634-record>footer b{color:#344961;font-size:9px}
    .cv634-final{padding:100px 0;text-align:center;background:radial-gradient(circle at 50% 0,rgba(65,126,245,.2),transparent 42%),#06142c}.cv634-final span{color:#79aaff;font-size:11px;font-weight:900;text-transform:uppercase;letter-spacing:.12em}.cv634-final h2{max-width:850px;margin:14px auto;color:#fff!important;font-size:clamp(40px,5vw,65px);line-height:1.04;letter-spacing:-.05em}.cv634-final p{color:#b8c8dd!important;font-size:16px}.cv634-final .mk-actions{justify-content:center;margin-top:28px}
    @keyframes cv634-health{0%,16%{content:"—"}24%,92%{content:"72"}100%{content:"—"}}@keyframes cv634-blockers{0%,23%{content:"—"}31%,92%{content:"2"}100%{content:"—"}}@keyframes cv634-alerts{0%,28%{content:"—"}36%,92%{content:"7"}100%{content:"—"}}@keyframes cv634-alts{0%,34%{content:"—"}42%,92%{content:"18"}100%{content:"—"}}@keyframes cv634-posture{0%,18%{color:#b26a08;background:#fff3db}19%,48%{color:#b26a08;background:#fff3db}49%,68%{color:#c13a41;background:#fff0f1}69%,100%{color:#14834f;background:#e9f8f0}}@keyframes cv634-pulse{0%{box-shadow:0 0 0 0 rgba(29,173,107,.45)}70%{box-shadow:0 0 0 7px rgba(29,173,107,0)}100%{box-shadow:0 0 0 0 rgba(29,173,107,0)}}@keyframes cv634-progress{0%,5%{width:0}18%{width:21%}36%{width:43%}56%{width:64%}76%,94%{width:86%}100%{width:0}}@keyframes cv634-risk-in{0%,18%{opacity:.2;transform:translateX(-8px)}26%,83%{opacity:1;transform:none}95%,100%{opacity:.2;transform:translateX(-8px)}}@keyframes cv634-answer{0%,40%{opacity:0;transform:translateY(5px)}49%,84%{opacity:1;transform:none}94%,100%{opacity:0;transform:translateY(5px)}}@keyframes cv634-decision-in{0%,62%{opacity:0;transform:translateY(5px)}70%,91%{opacity:1;transform:none}100%{opacity:0;transform:translateY(5px)}}@keyframes cv634-event{0%,18%{opacity:.28;transform:translateY(4px)}28%,78%{opacity:1;transform:none;border-color:#9fc0f6;box-shadow:0 5px 14px rgba(47,109,246,.09)}92%,100%{opacity:.28;transform:translateY(4px)}}@keyframes cv634-caption{0%,8%{opacity:0;transform:translateY(5px)}12%,24%{opacity:1;transform:none}28%,100%{opacity:0;transform:translateY(-5px)}}@keyframes cv634-flow{0%{transform:translateX(0)}50%{transform:translateX(355%)}100%{transform:translateX(0)}}@keyframes cv634-retrieve{0%{background-size:0 100%}60%,100%{background-size:100% 100%}}
    @media(max-width:1250px){.cv634-hero-grid{grid-template-columns:1fr}.cv634-copy h1{max-width:820px}.cv634-title-line{white-space:normal}.cv634-product{transform:none}.cv634-command-grid,.cv634-split,.cv634-decision-grid{grid-template-columns:1fr}.cv634-split,.cv634-decision-grid{gap:45px}}@media(max-width:900px){.cv634-pipeline{grid-template-columns:repeat(2,1fr)}.cv634-flowline{display:none}.cv634-supplier-table{overflow-x:auto}.cv634-supplier-table header,.cv634-supplier-table>div{min-width:820px}.cv634-supplier-kpis{grid-template-columns:repeat(2,1fr)}.cv634-record-meta{grid-template-columns:1fr}.cv634-record-meta>div{border-right:0;border-bottom:1px solid #e1e8ef}}@media(max-width:650px){.cv634-hero{padding:52px 0}.cv634-copy h1{font-size:42px;line-height:1.03}.cv634-title-line{white-space:normal}.cv634-app{grid-template-columns:1fr}.cv634-app aside{display:none}.cv634-kpis{grid-template-columns:repeat(2,1fr)}.cv634-workarea{grid-template-columns:1fr}.cv634-product{overflow:visible}.cv634-pipeline{grid-template-columns:1fr}.cv634-command-metrics,.cv634-supplier-kpis{grid-template-columns:1fr}.cv634-exposure>div{grid-template-columns:110px 1fr 45px}.cv634-coverage .mk-wrap{flex-wrap:wrap;gap:18px;padding:18px 0}.cv634-section{padding:78px 0}.cv634-supplier-action{align-items:flex-start;flex-direction:column}.cv634-demo-caption{display:none}}
    @media(prefers-reduced-motion:reduce){.cv634-story *{animation:none!important;transition:none!important}.cv634-risk,.cv634-answer,.cv634-decision,.cv634-live-events span{opacity:1!important;transform:none!important}.cv634-health::after{content:"72"}.cv634-product{transform:none}}
    </style>
    """, unsafe_allow_html=True)


def _sprint_63_5_design_excellence_css() -> None:
    """Design excellence and explanatory micro-interactions for Sprint 63.5."""
    st.markdown("""
    <style id="cadivor-sprint-63-5-design-excellence">
    /* Rhythm and hierarchy */
    .cv634-story{--cv635-ease:cubic-bezier(.22,.8,.24,1)}
    .cv634-section{padding:88px 0!important}
    .cv634-heading{max-width:820px;margin-bottom:42px!important}
    .cv634-heading>span,.cv634-copy-block>span,.cv634-decision-copy>span{font-size:11px!important;letter-spacing:.15em!important}
    .cv634-heading h2,.cv634-copy-block h2,.cv634-decision-copy h2{font-size:clamp(40px,4.35vw,62px)!important;line-height:1.02!important;letter-spacing:-.052em!important}
    .cv634-heading p,.cv634-copy-block>p,.cv634-decision-copy>p{font-size:17px!important;line-height:1.72!important}
    .cv634-coverage{border-top:1px solid #e4eaf2;border-bottom:1px solid #e4eaf2}
    .cv635-dark-stage{background:radial-gradient(circle at 82% 18%,rgba(58,115,229,.16),transparent 32%),linear-gradient(145deg,#06142c,#0a2146)!important}
    .cv635-light-stage{background:linear-gradient(180deg,#fff,#f7faff)!important}
    .cv635-soft-stage{background:radial-gradient(circle at 12% 20%,rgba(47,109,246,.07),transparent 28%),#eef3f9!important}
    .cv635-paper-stage{background:linear-gradient(180deg,#f7f9fc,#fff)!important}

    /* Hero composition */
    .cv634-hero{padding:78px 0 76px!important}
    .cv634-hero-grid{grid-template-columns:minmax(500px,.86fr) minmax(720px,1.34fr)!important;gap:54px!important}
    .cv634-copy h1{font-size:clamp(45px,3.75vw,59px)!important;line-height:1.015!important;max-width:720px!important}
    .cv634-copy>p{font-size:17px!important;max-width:620px!important}
    .cv634-product{box-shadow:0 50px 120px rgba(0,0,0,.42),0 0 0 1px rgba(115,163,242,.08)!important;transition:transform .45s var(--cv635-ease),box-shadow .45s var(--cv635-ease)!important}
    @media(hover:hover){.cv634-product:hover{transform:perspective(1500px) rotateY(-.5deg) translateY(-5px)!important;box-shadow:0 62px 140px rgba(0,0,0,.47),0 0 0 1px rgba(115,163,242,.16)!important}}
    .cv635-telemetry{display:flex;align-items:center;justify-content:space-between;max-width:590px;margin-top:20px;padding-top:16px;border-top:1px solid rgba(255,255,255,.1);color:#8fa4c2;font-size:10px;font-weight:750}
    .cv635-telemetry span{display:flex;align-items:center;gap:8px}.cv635-telemetry i{width:7px;height:7px;border-radius:50%;background:#38d58b;box-shadow:0 0 0 0 rgba(56,213,139,.5);animation:cv635-live 1.8s infinite}.cv635-telemetry b{color:#c9d8ec;font-weight:750}
    .cv634-canvas{position:relative}
    .cv635-status-rail{display:grid;grid-template-columns:repeat(5,1fr);gap:5px;margin:8px 0 10px}
    .cv635-status-rail span{position:relative;padding:6px 4px 6px 14px;border:1px solid #e0e7f0;border-radius:7px;background:#fff;color:#7b899b;font-size:6px;font-weight:800;opacity:.45;overflow:hidden}
    .cv635-status-rail span::before{content:"";position:absolute;left:5px;top:50%;width:5px;height:5px;border-radius:50%;background:#cbd5e1;transform:translateY(-50%)}
    .cv635-status-rail .s1{animation:cv635-status 12s .4s infinite}.cv635-status-rail .s2{animation:cv635-status 12s 1.7s infinite}.cv635-status-rail .s3{animation:cv635-status 12s 3s infinite}.cv635-status-rail .s4{animation:cv635-status 12s 4.5s infinite}.cv635-status-rail .s5{animation:cv635-status-green 12s 8.2s infinite}
    .cv634-kpis article{position:relative;overflow:hidden;transition:transform .2s var(--cv635-ease),border-color .2s var(--cv635-ease)!important}
    .cv634-kpis article::after{content:"";position:absolute;left:0;bottom:0;width:0;height:2px;background:#2f6df6;animation:cv635-kpi-progress 12s infinite}
    .cv634-kpis article:nth-child(2)::after{animation-delay:.45s;background:#e0474c}.cv634-kpis article:nth-child(3)::after{animation-delay:.8s;background:#e09a22}.cv634-kpis article:nth-child(4)::after{animation-delay:1.1s;background:#18a865}
    .cv634-risks,.cv634-insight{box-shadow:0 10px 24px rgba(15,23,42,.055)}

    /* Connected workflow */
    .cv634-workflow{background:#fff!important}
    .cv634-pipeline{grid-template-columns:repeat(5,1fr)!important;gap:12px!important;position:relative}
    .cv634-pipeline article{min-height:190px!important;padding:24px 20px!important;border-radius:16px!important;transition:transform .24s var(--cv635-ease),border-color .24s var(--cv635-ease),box-shadow .24s var(--cv635-ease)!important;animation:cv635-pipeline-focus 12s infinite}
    .cv634-pipeline article:nth-of-type(2){animation-delay:1.8s}.cv634-pipeline article:nth-of-type(3){animation-delay:3.6s}.cv634-pipeline article:nth-of-type(4){animation-delay:5.4s}.cv634-pipeline article:nth-of-type(5){animation-delay:7.2s}
    .cv634-flowline i{animation-duration:12s!important}
    @media(hover:hover){.cv634-pipeline article:hover{transform:translateY(-6px);border-color:#91b5f0!important;box-shadow:0 24px 50px rgba(22,55,106,.12)!important}}

    /* Command center as one surface */
    .cv634-command-grid{gap:18px!important;padding:10px;border:1px solid rgba(126,163,222,.18);border-radius:24px;background:rgba(255,255,255,.045);box-shadow:0 36px 85px rgba(0,0,0,.26)}
    .cv634-command-main,.cv634-action-rail{border-radius:17px!important}
    .cv634-command-metrics article{position:relative;overflow:hidden}
    .cv634-command-metrics article::after{content:"";position:absolute;left:0;bottom:0;height:3px;width:var(--meter,84%);background:linear-gradient(90deg,#2f6df6,#75a7ff);transform-origin:left;animation:cv635-meter 1.1s var(--cv635-ease) both}
    .cv634-command-metrics article:nth-child(2){--meter:42%}.cv634-command-metrics article:nth-child(2)::after{background:linear-gradient(90deg,#e0474c,#ff8b8f)}.cv634-command-metrics article:nth-child(3){--meter:76%}.cv634-command-metrics article:nth-child(3)::after{background:linear-gradient(90deg,#18a865,#65df9f)}
    .cv634-action-rail article{transition:transform .2s var(--cv635-ease),background .2s var(--cv635-ease)!important}.cv634-action-rail article:hover{transform:translateX(5px);background:#f4f8ff!important}

    /* Copilot explanatory motion */
    .cv634-copilot-ui{box-shadow:0 30px 75px rgba(18,47,92,.15)!important;transition:transform .35s var(--cv635-ease),box-shadow .35s var(--cv635-ease)!important}
    @media(hover:hover){.cv634-copilot-ui:hover{transform:translateY(-5px);box-shadow:0 40px 90px rgba(18,47,92,.2)!important}}
    .cv634-retrieval b{opacity:.36;transform:translateY(3px);animation:cv635-evidence 8s infinite}
    .cv634-retrieval b:nth-of-type(2){animation-delay:.6s}.cv634-retrieval b:nth-of-type(3){animation-delay:1.2s}.cv634-retrieval b:nth-of-type(4){animation-delay:1.8s}
    .cv634-ai{position:relative;overflow:hidden}.cv634-ai::before{content:"";position:absolute;left:0;top:0;width:3px;height:0;background:#75a7ff;animation:cv635-ai-line 8s 2.4s infinite}

    /* Supplier workspace */
    .cv634-supplier-ui{border-radius:22px!important;box-shadow:0 35px 85px rgba(17,43,82,.14)!important;overflow:hidden}
    .cv634-supplier-table>div{transition:background .18s var(--cv635-ease),transform .18s var(--cv635-ease)!important}.cv634-supplier-table>div:hover{background:#edf4ff!important;transform:translateX(3px)}
    .cv634-supplier-table>div:nth-child(3){animation:cv635-supplier-select 8s 2.2s infinite}
    .cv634-supplier-action{position:relative;overflow:hidden}.cv634-supplier-action::after{content:"";position:absolute;left:0;bottom:0;height:3px;width:91%;background:linear-gradient(90deg,#2f6df6,#68a0ff);animation:cv635-meter 1.2s .3s var(--cv635-ease) both}

    /* Decision record as the climax */
    .cv634-decision-grid{align-items:center!important;gap:78px!important}
    .cv634-record{border-radius:6px!important;box-shadow:0 36px 80px rgba(27,47,75,.18),0 0 0 1px #d9e2ec!important;transform:rotate(.25deg);transition:transform .35s var(--cv635-ease),box-shadow .35s var(--cv635-ease)!important}
    .cv634-record::before{content:"CADIVOR · VERIFIED ENGINEERING RECORD";display:block;padding:9px 24px;border-bottom:1px solid #dfe6ee;background:#f5f7fa;color:#7b8899;font-size:7px;font-weight:900;letter-spacing:.16em}
    @media(hover:hover){.cv634-record:hover{transform:rotate(0) translateY(-5px);box-shadow:0 46px 100px rgba(27,47,75,.23),0 0 0 1px #cbd8e6!important}}
    .cv634-record>header span{animation:cv635-approved 2.6s infinite}
    .cv634-record>footer div{opacity:.55;animation:cv635-timeline 7s infinite}.cv634-record>footer div:nth-child(2){animation-delay:.8s}.cv634-record>footer div:nth-child(3){animation-delay:1.6s}
    .cv634-final{padding:92px 0!important}

    @keyframes cv635-live{0%{box-shadow:0 0 0 0 rgba(56,213,139,.45)}70%{box-shadow:0 0 0 8px rgba(56,213,139,0)}100%{box-shadow:0 0 0 0 rgba(56,213,139,0)}}
    @keyframes cv635-status{0%,8%,100%{opacity:.45;border-color:#e0e7f0;color:#7b899b}12%,26%{opacity:1;border-color:#93b8f2;color:#2259be;box-shadow:0 5px 13px rgba(47,109,246,.1)}12%,26%{background:#f4f8ff}14%,24%{transform:translateY(-1px)}}
    @keyframes cv635-status-green{0%,66%,100%{opacity:.45}72%,92%{opacity:1;border-color:#8bd5ae;background:#effaf4;color:#14834f;box-shadow:0 5px 13px rgba(24,168,101,.11)}}
    @keyframes cv635-kpi-progress{0%,12%{width:0}35%,88%{width:100%}100%{width:0}}
    @keyframes cv635-pipeline-focus{0%,12%,100%{transform:none;border-color:#dce5f0;box-shadow:0 15px 38px rgba(15,23,42,.055)}16%,28%{transform:translateY(-5px);border-color:#8eb4f2;box-shadow:0 23px 50px rgba(25,65,128,.13)}}
    @keyframes cv635-meter{from{transform:scaleX(0)}to{transform:scaleX(1)}}
    @keyframes cv635-evidence{0%,8%,100%{opacity:.36;transform:translateY(3px)}14%,72%{opacity:1;transform:none;background:#e8f1ff;border-color:#a6c4f4;color:#2359b4}}
    @keyframes cv635-ai-line{0%,8%{height:0}24%,76%{height:100%}90%,100%{height:0}}
    @keyframes cv635-supplier-select{0%,20%,100%{background:#fff;box-shadow:none}28%,68%{background:#edf4ff;box-shadow:inset 3px 0 0 #2f6df6}}
    @keyframes cv635-approved{0%,100%{box-shadow:0 0 0 0 rgba(24,168,101,.25)}50%{box-shadow:0 0 0 7px rgba(24,168,101,0)}}
    @keyframes cv635-timeline{0%,8%,100%{opacity:.55}18%,68%{opacity:1}}
    @media(max-width:1300px){.cv634-hero-grid{grid-template-columns:1fr!important}.cv634-copy h1{max-width:820px!important}.cv634-pipeline{grid-template-columns:repeat(3,1fr)!important}}
    @media(max-width:900px){.cv634-pipeline{grid-template-columns:repeat(2,1fr)!important}.cv635-status-rail{grid-template-columns:1fr 1fr}.cv634-decision-grid{gap:45px!important}.cv634-record{transform:none}}
    @media(max-width:620px){.cv634-section{padding:64px 0!important}.cv634-heading h2,.cv634-copy-block h2,.cv634-decision-copy h2{font-size:36px!important}.cv634-pipeline{grid-template-columns:1fr!important}.cv635-status-rail{display:none}.cv635-telemetry{align-items:flex-start;flex-direction:column}.cv634-final{padding:72px 0!important}}
    @media(prefers-reduced-motion:reduce){.cv635-telemetry i,.cv635-status-rail span,.cv634-kpis article::after,.cv634-pipeline article,.cv634-command-metrics article::after,.cv634-retrieval b,.cv634-ai::before,.cv634-supplier-table>div,.cv634-supplier-action::after,.cv634-record>header span,.cv634-record>footer div{animation:none!important}.cv634-retrieval b{opacity:1!important;transform:none!important}}
    </style>
    """, unsafe_allow_html=True)
...
def _sprint_63_5_2_interactive_polish_css() -> None:
    """Reduce secondary hero scale and keep animated product stories continuously populated."""
    st.markdown(r"""
    <style id="cadivor-sprint-63-5-2-interactive-polish">
    /* Secondary-page heroes: confident, readable, and never oversized. */
    .mk-page-hero{padding:68px 0 64px!important}
    .mk-page-hero-grid{grid-template-columns:minmax(500px,.92fr) minmax(500px,1.08fr)!important;gap:54px!important}
    .mk-page-hero h1{font-size:clamp(38px,3.25vw,52px)!important;line-height:1.06!important;letter-spacing:-.045em!important;max-width:690px!important;text-wrap:balance}
    .mk-page-hero p{font-size:16px!important;line-height:1.66!important;max-width:680px!important}
    .mk-page-hero .mk-eyebrow{margin-bottom:18px!important}
    .mk-page-hero .mk-actions{margin-top:23px!important;margin-bottom:18px!important}
    .mk-page-hero .mk-hero-visual{min-height:350px!important}

    /* Never hide a complete section while waiting for viewport detection. */
    .mk-launch-story>section:not(.mk-launch-hero){opacity:1!important;transform:none!important}
    .mk-launch-story>section .cv634-heading,.mk-launch-story>section .cv634-copy-block,.mk-launch-story>section .cv634-decision-copy{transition:transform .5s ease,opacity .5s ease}

    /* BOM upload demonstration: visible at all times, with activity layered over the product. */
    .cv6352-bom-demo{position:relative!important;overflow:hidden!important;padding-top:76px!important}
    .cv6352-upload-card{position:absolute;z-index:8;left:20px;right:20px;top:17px;display:grid;grid-template-columns:38px 1fr auto;align-items:center;gap:11px;padding:10px 12px;border:1px solid #dce6f3;border-radius:11px;background:rgba(255,255,255,.97);box-shadow:0 12px 28px rgba(22,55,99,.12)}
    .cv6352-file-icon{display:grid;place-items:center;width:38px;height:38px;border-radius:9px;background:#e9f2ff;color:#2563eb;font-size:8px;font-weight:900;letter-spacing:.05em}
    .cv6352-upload-card b,.cv6352-upload-card small{display:block}.cv6352-upload-card b{font-size:11px;color:#0d1b32}.cv6352-upload-card small{margin-top:3px;font-size:8px;color:#75859b}.cv6352-upload-card em{font-size:8px;font-style:normal;font-weight:850;color:#2563eb}
    .cv6352-upload-progress{position:absolute;z-index:9;left:20px;right:20px;top:67px;height:3px;border-radius:999px;background:#edf2f8;overflow:hidden}.cv6352-upload-progress i{display:block;height:100%;width:100%;background:linear-gradient(90deg,#2f6df6,#69a2ff);transform-origin:left;animation:cv6352-upload 7.5s ease-in-out infinite}
    .cv6352-bom-demo>.mk-cockpit{animation:cv6352-dashboard-live 7.5s ease-in-out infinite}

    /* Ask Cadivor: keep the interface populated and animate typing/reasoning inside it. */
    .cv6351-chat-user{opacity:1!important;transform:none!important;animation:none!important;min-height:58px}
    .cv6351-chat-user strong{display:flex!important;align-items:center;min-height:20px;overflow:hidden}
    .cv6352-typed-question{display:inline-block;white-space:nowrap;overflow:hidden;width:0;animation:cv6352-type-question 9s steps(40,end) infinite}
    .cv6352-caret,.cv6352-ai-caret{display:inline-block;width:1px;height:14px;margin-left:3px;background:#8cb6f8;animation:cv6352-caret .7s step-end infinite}
    .cv6351-thinking{min-height:40px;opacity:.72!important;animation:cv6352-thinking-shell 9s infinite!important}
    .cv6352-thinking-label::after{content:' · BOM';animation:cv6352-reason-label 9s steps(1,end) infinite}
    .cv6351-evidence span{opacity:.42!important;transform:none!important}
    .cv6351-evidence .e1{animation:cv6352-evidence 9s .9s infinite!important}.cv6351-evidence .e2{animation:cv6352-evidence 9s 1.55s infinite!important}.cv6351-evidence .e3{animation:cv6352-evidence 9s 2.2s infinite!important}.cv6351-evidence .e4{animation:cv6352-evidence 9s 2.85s infinite!important}
    .cv6351-chat-ai{display:block!important;min-height:202px;opacity:1!important;transform:none!important;animation:cv6352-answer-shell 9s infinite!important}
    .cv6351-chat-ai>strong{display:flex!important;align-items:center;min-height:23px;overflow:hidden}
    .cv6352-typed-answer{display:inline-block;white-space:nowrap;overflow:hidden;width:0;animation:cv6352-type-answer 9s steps(30,end) infinite}
    .cv6351-chat-ai>p,.cv6351-chat-ai>div{animation:cv6352-answer-detail 9s infinite}
    .cv6351-followup{min-height:52px;opacity:.35!important;transform:none!important;animation:cv6352-followup 9s infinite!important}

    /* Add purposeful motion to otherwise static product visuals. */
    .mk-hero-visual .mk-visual-row:nth-child(1){animation:cv6352-row-focus 8s 0s infinite}.mk-hero-visual .mk-visual-row:nth-child(2){animation:cv6352-row-focus 8s 1.4s infinite}.mk-hero-visual .mk-visual-row:nth-child(3){animation:cv6352-row-focus 8s 2.8s infinite}.mk-hero-visual .mk-visual-row:nth-child(4){animation:cv6352-row-focus 8s 4.2s infinite}
    .mk-hero-visual .mk-visual-highlight{position:relative;overflow:hidden}.mk-hero-visual .mk-visual-highlight::after{content:'';position:absolute;inset:0;background:linear-gradient(100deg,transparent 25%,rgba(255,255,255,.16) 48%,transparent 70%);transform:translateX(-120%);animation:cv6352-shimmer 6s infinite}

    @keyframes cv6352-upload{0%,8%{transform:scaleX(.05)}38%,68%{transform:scaleX(1)}82%,100%{transform:scaleX(.05)}}
    @keyframes cv6352-dashboard-live{0%,18%{filter:saturate(.72);transform:translateY(2px)}38%,78%{filter:saturate(1);transform:none}100%{filter:saturate(.72);transform:translateY(2px)}}
    @keyframes cv6352-type-question{0%,7%{width:0}30%,82%{width:40ch}92%,100%{width:40ch}}
    @keyframes cv6352-caret{50%{opacity:0}}
    @keyframes cv6352-thinking-shell{0%,28%{opacity:.42}34%,52%{opacity:1;border-color:rgba(117,167,255,.35)}58%,100%{opacity:.5}}
    @keyframes cv6352-reason-label{0%,32%{content:' · BOM'}38%,43%{content:' · lifecycle'}49%,54%{content:' · suppliers'}60%,100%{content:' · alternatives'}}
    @keyframes cv6352-evidence{0%,25%,100%{opacity:.42;background:rgba(255,255,255,.035);border-color:rgba(132,169,227,.16)}31%,74%{opacity:1;background:rgba(85,217,150,.08);border-color:rgba(85,217,150,.42);transform:translateY(-1px)}}
    @keyframes cv6352-answer-shell{0%,47%{background:linear-gradient(145deg,#0a244b,#103269)}55%,88%{background:linear-gradient(145deg,#0c2a57,#123b78)}100%{background:linear-gradient(145deg,#0a244b,#103269)}}
    @keyframes cv6352-type-answer{0%,48%{width:0}66%,90%{width:31ch}100%{width:31ch}}
    @keyframes cv6352-answer-detail{0%,58%{opacity:.16;transform:translateY(3px)}68%,92%{opacity:1;transform:none}100%{opacity:.16;transform:translateY(3px)}}
    @keyframes cv6352-followup{0%,72%{opacity:.32}80%,94%{opacity:1;border-color:rgba(126,165,224,.38)}100%{opacity:.32}}
    @keyframes cv6352-row-focus{0%,14%,100%{transform:none;box-shadow:none}20%,34%{transform:translateX(3px);box-shadow:0 8px 20px rgba(37,99,235,.1)}}
    @keyframes cv6352-shimmer{0%,30%{transform:translateX(-120%)}58%,100%{transform:translateX(120%)}}
    @media(max-width:1180px){.mk-page-hero-grid{grid-template-columns:1fr!important}.mk-page-hero h1{max-width:820px!important}.mk-page-hero .mk-hero-visual{max-width:900px;width:100%;justify-self:center}}
    @media(max-width:620px){.mk-page-hero h1{font-size:38px!important}.cv6352-typed-question,.cv6352-typed-answer{white-space:normal!important;width:auto!important;animation:none!important}.cv6352-caret,.cv6352-ai-caret{display:none}.cv6352-bom-demo{padding-top:88px!important}.cv6352-upload-card{grid-template-columns:34px 1fr}.cv6352-upload-card em{display:none}}
    @media(prefers-reduced-motion:reduce){.cv6352-upload-progress i,.cv6352-bom-demo>.mk-cockpit,.cv6352-typed-question,.cv6352-typed-answer,.cv6352-caret,.cv6352-ai-caret,.cv6351-thinking,.cv6351-evidence span,.cv6351-chat-ai,.cv6351-chat-ai>p,.cv6351-chat-ai>div,.cv6351-followup,.mk-hero-visual .mk-visual-row,.mk-hero-visual .mk-visual-highlight::after{animation:none!important}.cv6352-typed-question,.cv6352-typed-answer{width:auto!important}}
    </style>
    """, unsafe_allow_html=True)
...
def render_marketing_site(*, forced_page: str | None = None) -> None:
    ...
    _sprint_63_5_2_interactive_polish_css()
