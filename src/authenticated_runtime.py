Warning: truncated output (original token count: 152850)
Total output lines: 13349
...
def generate_bom_pdf_report(project_name, selected_parts, attention_parts, bom_health_score, alternative_history=None):
    ...
    if alternative_history is None or alternative_history.empty:
            ...
            for original_part, group_df in true_alternative_history.groupby("original_part"):
                ...
                for _, row in group_df.head(5).iterrows():
                    engineering_text = (
                        ...
                        f"Compatibility Notes: {row.get('compatibility…122850 tokens truncated…#b91c1c;
                font-weight:900;
            }
        ...
        if total_high_risk and st.button(
            "Review high-risk saved analyses",
            key="bom81_review_high_risk",
            use_container_width=True,
        ):
            st.session_state["bom81_manager_sort"] = "High risk: high to low"
            st.session_state["bom81_high_risk_review_requested"] = True
        ...
        with st.container(key="bom81_saved_manager"):
            with st.expander(
                ...
                if st.session_state.pop("bom81_high_risk_review_requested", False):
                    st.info("Saved analyses are sorted by their high-risk component count.")
